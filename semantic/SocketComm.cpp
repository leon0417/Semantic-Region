...
CSocketComm::CSocketComm() :
    ...
    m_hComm(INVALID_HANDLE_VALUE), m_hThread(NULL), m_hMutex(NULL), m_pWOwner(NULL)
{
	lpData  = (LPBYTE)&stMsgProxy;
	dwSize  = sizeof(stMsgProxy);
	m_bConnect= false;
}
...
{
	//BYTE str[100] = "asfasddfasf";
	//WriteComm( str, sizeof(str), 0 );
	::SendMessage(m_pWOwner->m_hWnd, WM_USER_SOCKET_REV_MESSAGE, (WPARAM) lpBuffer, (LPARAM) dwCount);
}
